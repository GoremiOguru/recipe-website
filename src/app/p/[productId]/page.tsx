...
export default async function ProductScreen({params}: Props) {
  ...
  if (!data) 
    ...
    <div className="flex justify-center items-center min-h-dvh">
      <p className="text-2xl animate-bounce text-red-500">Product not found</p>
    </div>
  );
  ...
  return(
    <div className="flex flex-col gap-8 py-4 px-8 w-full max-w-[1250px] mx-auto" >
      ...
      <div className=" grid md:grid-cols-2 gap-4">
        ...
        <Card className=" bg-card border border-border p-4 flex flex-col gap-4">
          ...
          <div className=" flex flex-col gap-4">
            <span>{formatMoney(data.rating,"NGN" ,0)}</span>
          </div>
          <Separator/>
